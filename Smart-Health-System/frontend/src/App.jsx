...
═══════════════════════════════════════════════════ */
const D = {
  bg:"#080E1A", surface:"#0D1627", surface2:"#111E33", surface3:"#162340",
  border:"rgba(56,189,248,0.1)", border2:"rgba(255,255,255,0.06)",
  text:"#F1F5F9", textMid:"#94A3B8", textDim:"#475569",
  critical:"#F43F5E", high:"#FB923C", medium:"#FBBF24", low:"#34D399", info:"#38BDF8",
};
...
function useMLRisk() {
  ...
  async function fetchRisk() {
    ...
    try {
      const res  = await fetch(ML_ENDPOINT, {
        ...
        body:    JSON.stringify({
          ward_data: [
            { ward:"Ukkadam",     contamination:58, cases:8,  rainfall:12 },
            { ward:"Singanallur", contamination:52, cases:6,  rainfall:8  },
            { ward:"Peelamedu",   contamination:45, cases:5,  rainfall:15 },
            { ward:"Ganapathy",   contamination:42, cases:4,  rainfall:10 },
          ]
        }),
      });
      if (!res.ok) throw new Error("HTTP " + res.status);
      const data = await res.json();
      // Normalise: accept "high"→"danger", "low"→"safe", or direct values
      const raw = (data.risk || "medium").toLowerCase();
      const normalised = raw === "high" || raw === "danger" ? "danger"
                       : raw === "low"  || raw === "safe"   ? "safe"
                       : "medium";
      setRisk(normalised);
      if (data.confidence != null) setConfidence(Math.round(data.confidence));
      if (data.cases      != null) setCases(data.cases);
      if (data.peak       != null) setPeak(data.peak);
      setLastUpdate(new Date());
    } catch (err) {
      setError(err.message);
      // keep previous values — don't reset to defaults on error
    } finally {
      setLoading(false);
    }
  }
  ...
  useEffect(() => {
    fetchRisk();                          // fetch on mount
    const id = setInterval(fetchRisk, 30_000); // re-fetch every 30 s
...
function Card({ children, style={}, glow }) {
  return (
    <div style={{ background:D.surface, borderRadius:16, border:"1px solid "+D.border,
      padding:20, position:"relative", overflow:"hidden",
      boxShadow: glow
        ? "0 0 40px "+glow+"18, 0 4px 24px rgba(0,0,0,0.3)"
  ...
  return (
    <div style={{ fontFamily:"'DM Sans','Segoe UI',sans-serif",
      ...
      <div style={{ position:"fixed", inset:0,
        backgroundImage:"radial-gradient(rgba(56,189,248,0.025) 1px, transparent 1px)",
        backgroundSize:"28px 28px", pointerEvents:"none", zIndex:0 }}/>

      {/* ── TOPBAR ── */}
      <div style={{ position:"sticky", top:0, zIndex:50, width:"100%", height:60,
        background:D.surface+"ee", backdropFilter:"blur(16px)",
        ...
        <div style={{ width:"100%", maxWidth:1440,
          ...
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            <div style={{ width:34, height:34, borderRadius:10,
              ...
              display:"flex", alignItems:"center", justifyContent:"center",
              fontSize:17, boxShadow:"0 0 20px rgba(14,165,233,0.4)", flexShrink:0 }}>💧</div>
            <div>
              <div style={{ fontSize:isMobile?13:15, fontWeight:800,
                letterSpacing:"-0.02em", color:D.text }}>AquaWatch CBE</div>
              {!isMobile && <div style={{ fontSize:11, color:D.textMid }}>
                Coimbatore District — Water-Borne Disease Surveillance</div>}
            </div>
          </div>
          <div style={{ display:"flex", alignItems:"center", gap:isMobile?8:14 }}>
            <div style={{ display:"flex", alignItems:"center", gap:5,
              background:"rgba(52,211,153,0.1)", border:"1px solid rgba(52,211,153,0.3)",
              borderRadius:999, padding:isMobile?"4px 10px":"5px 13px",
              fontSize:12, fontWeight:600, color:D.low }}>
              <span style={{ width:7, height:7, borderRadius:"50%", background:D.low,
                animation:"pulse 2s infinite", display:"inline-block",
                boxShadow:"0 0 8px "+D.low, flexShrink:0 }}/>
              {isMobile ? "Live" : "Live Monitoring"}
            </div>
            {!isMobile && <div style={{ fontSize:12, color:D.textMid,
              fontVariantNumeric:"tabular-nums" }}>{clock}</div>}
          </div>
        </div>
      </div>
